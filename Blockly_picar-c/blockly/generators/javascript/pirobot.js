...
Blockly.JavaScript['robot_light'] = function(block) {
	var value_light = Blockly.JavaScript.valueToCode(block, 'LIGHT',
			Blockly.JavaScript.ORDER_ATOMIC);
	var dropdown_color = block.getFieldValue('COLOR');
	
    // Check the input ...
    if (value_light > 255) {
        alert("Maximum lightness for the LED is 255.");
        return '';
    }
    if (value_light < 0) {
        alert("Minimum lightness for the LED is 0.");
        return '';
    }
    // Generate the code ...
	var code = 'runPiRobotCommand("setRobotMotorPower", "' + dropdown_color + '",' + value_light + ');';
	return code;
};
